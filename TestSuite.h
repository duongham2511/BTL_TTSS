...
class TestSuite{
    ...
    static void StrassenOMP_03Test(int rowA, int colA_rowB, int colB, int attempts)
    {
        double **A = Utility::AllocateMemory2D<double>(rowA,colA_rowB);
        double **B = Utility::AllocateMemory2D<double>(colA_rowB,colB);
        double **C;
        ...
        printf("%d threads available for parallel processing.\n",omp_get_max_threads());
        printf("Multiplying a %d x %d matrix with a %d x %d matrix using Strassen OMP alt 2, %d try.\n",rowA, colA_rowB, colA_rowB, colB, attempts);
